...
const peopleDataReducers = (state, action)=>{
    if(action.type == 'name'){
        return {...state, person:{...state.person, name:action.name}}
        // return {person:{name:action.name, age:state.person.age,}, peopleList:state.peopleList}
    } else if (action.type == 'age'){
        return { ...state, person: { ...state.person, age: action.age } }
        // return { person: { name: state.person.name, age: action.age, }, peopleList: state.peopleList }
    }else if(action.type == 'addlist'){
        return { ...state, peopleList: [...state.peopleList, {name:state.person.name, age:state.person.age}], person:initial.person }
    }
}

function TestReducersCleaner() {
...
export default TestReducersCleaner